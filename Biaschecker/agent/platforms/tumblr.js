...
class TwitterSearch {
    ...
    async doLogin() {
        ...
        const LOGIN_INPUT_SELECTOR = 'input.gj_Aq';
    ...
    async doQuery(term) {
        ...
        const SEARCH_INPUT_SELECTOR = 'input[class="NaqPB"]';
    ...
    async fetchResults(term, maxResults) {
        ...
        const new_RELATED = 'aside > div:first-child div[class="So6RQ YZ6YQ"]';


        const new_PEOPLE_SELECTOR = 'aside > div:last-child div[class="So6RQ YZ6YQ"]';


        const old_RELATED = 'div.loL53  a';

        const POST_SELECTOR = 'article';

        const old_PEOPLE_SELECTOR = 'a[class="BSUG4 lk3RB J_gJ7 lpoBh"]';
        
        await new Promise(resolve => setTimeout(() => resolve(), 2000));
        
        
        try {
            await this.page.waitForSelector(old_RELATED, {
                timeout: 30000
            });
            await this.page.waitForSelector(POST_SELECTOR, {
                timeout: 10000
            });
            await this.page.waitForSelector(old_PEOPLE_SELECTOR, {
                timeout: 10000
            });
            await this.page.waitForSelector(new_RELATED, {
                timeout: 10000
            });
            await this.page.waitForSelector(new_PEOPLE_SELECTOR, {
                timeout: 10000
            });
        } catch (e) {
            console.log(e);
        ...
        const e1=await this.page.$(old_RELATED);

        const e2=await this.page.$(new_RELATED);
        console.log(e1);
        console.log(e2);
        ...
        if (e1 && !e2 ){
            console.log('old');
            RELATED=old_RELATED;
            PEOPLE_SELECTOR=old_PEOPLE_SELECTOR;
        }else{
            console.log('new');
            RELATED=new_RELATED;
            PEOPLE_SELECTOR=new_PEOPLE_SELECTOR;
        }
    ...
    async doFollow(profile) {
        ...
        const LIKE_BUTTON_SELECTOR = 'div[class="like_button"]';

        const FOLLOW_BUTTON_SELECTOR = 'button[class="TRX6J"]';
